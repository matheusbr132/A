...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5566997250299:+55 (66) 997250299\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5566997250299@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5566997250299@s.whatsapp.net'
                        /******ApiKey Input******/
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Pedido recebido, Baiano removido da posição de administrador :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Pedido recebido, Baiano removido da posição de administrador @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                                        gatauda = body.slice(6)
                                        if (!isUser) return reply(mess.only.B)
                                        reply(mess.wait)
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.B)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall numero da sua mãe...`)
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Baiano burro do KRL')
                                        if (!isUser) return reply(mess.only.B)
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`Digite as perguntas\Exemploh : ${prefix}kbbi seu cartão e os ultimos 3 digitos`)
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                if (!iUser) return reply(mess.only.B)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Baiano Burro Cade o texto', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.B)
					if (!isGroup) return reply(mess.only.group)
					teks = `Baianos chefes*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'chord':
					if (args.length < 1) return reply('Cade baiano o titulo da musuca?')
                                        if (!isUser) return reply(mess.only.B)
					tels = body.slice(7)					
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O Baiano chefe é o : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca-alvo que você quer chutar! de preferencia seja um baiano')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, o texto é muito longo...', msgType.text, {quoted: mek})
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Algum baiano, ou polaco fechou o grupo @${nomor.split("@s.whatsapp.net")[0]}\nagora *apenas baianos* podem enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
                ...
                case 'bukagc':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					open = {
					text: `Grupo aberto pelos baianos @${sender.split("@")[0]}\nagora *todos os normies* podem enviar mensagens`,
					contextInfo: { mentionedJid: [sender] }
					}
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.B)
					if (!isQuotedSticker) return reply('❌ Esses baianos, apenas Stickers ❌')
					reply(mess.wait)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Parâmetro incorreto \nCommand : ${prefix} nome|idade\nContoh : ${prefix} Toin|18`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`Um baiano se regiatrou, SN: BAIANO.EXEC\`\`\`\n\n\`\`\`Pada ${date} ${time}\`\`\`\n\`\`\`[Nama]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Era]: ${umure}\`\`\`\n\`\`\`Para usar o bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.B)
            ...
            break
                                ...
                                case 'nsfwtrap':
                                        try{
                                                if (!isNsfw) return reply('❌ *O Gustavo Desativiu o NS* ❌')
                                                if (!isUser) return reply(mess.only.B)
                                                ...
                                                console.log(`*Error* :`, color(e,'red'))
                                                reply('❌ *Deu erro baiani* ❌')
                                        }
										break
										case 'randomhentaio': 
						try {
							if (!isNsfw) return reply('❌ *O Gustavo desativou o NSFW* ❌')
				...
				case 'nsfwass':
					...
					case 'nsfwarmpits':
						...
						try {
							...
							res = await fetchJson(`https://tobz-api.herokuapp.com/nsfwtrap?apikey=BotWeA`, {method: 'get'})
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Pack do Micael\nConfia'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK CCT ${tels} Não incomode blz?*`,
				...
				case 'Modobaiano':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Baiano está ativado')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Ativado com sucesso o modo baiano ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativando o modo baiano com sucesso neste grupo ✔️')
					} else {
